...
function closeall()
{
	if (bbtags[0]) {
		while (bbtags[0]) {
			...
			document.REPLIER.postText.value += "[/" + tagRemove + "]";
	...
	bbtags = new Array();
	document.REPLIER.postText.focus();
}
...
{
    document.REPLIER.postText.value += NewCode;
    document.REPLIER.postText.focus();
}
...
function tag_hr()
{
	document.REPLIER.postText.value += "\r\n[HR]\r\n";
}
...
{
	var isClose = false;
	var obj_ta = document.REPLIER.postText;
